...
public class Server
{
    ...
    private static void UDPReceiveCallback(IAsyncResult _result)
    {
        try
        {
            ...
            using (Packet _packet = new Packet(_data))
            {
                ...
                if (clients[_clientId].isConnected)
                {
                    if (_clientId == 0)
                    {
                        return;
    ...
    private static void InitializeServerData()
    {
        ...
        {
            { (int)ClientPackets.ClientKey, ServerHandle.WelcomeReceived },
            ...
            { (int)ClientPackets.joinServer, ServerHandle.JoinServer }
        };
        Debug.Log("Initialized packets.");
    }

    public static Client getClientFromKey(string _key)
    {

        for (int i = 1; i <= MaxPlayers; i++)
        {
            if (clients[i].ClientKey == _key)
            {
                return clients[i];
            }
        }
